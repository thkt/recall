...
**Embedding** — 検索のたびに20チャンクをembed: 結果セッション10件 + 最新10件。Ruri v3（310Mパラメータ）をmlx-rs + MLXでApple Silicon上で実行します。バッチ推論（batch=32）と長さソートによるpadding最小化。モデルは `recall model download` で明示的にダウンロードします。未ダウンロード時はFTS5のキーワード検索のみで動作します。
...
| `recall embed`（28kチャンク）           | 約11分（M3, batch=32）                     |
