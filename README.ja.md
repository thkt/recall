...
チャンク生成はQ&Aが0件でも完了を記録し、変更のない会話の本文再取得・チャンク化を省きます。会話の再解析で完了状態を解除します。チャンクと完了状態は一緒にcommitし、失敗・中断時は未処理のまま再試行できます。アップグレードでは既存のチャンクとembeddingを保持し、従来0件だった会話を一度処理します。

embeddingを持つ会話の更新は、embeddingが利用可能になるまで延期します。同じセッションIDを使う別パスからの置換も含め、保存済み本文・チャンク・embeddingを保持します。
