...
**検索** — `recall search` は構築済みインデックスを読むだけで、インデックスは作成しません。事前に `recall index` でリフレッシュしてください（セッション変更時の自動再インデックスは計画中で、まだ実装されていません）。空のインデックスを検索すると `No sessions indexed. Run recall index first.` を表示します。
