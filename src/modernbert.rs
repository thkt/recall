...
// Ported from candle_transformers::models::modernbert.
...
fn prepare_4d_attention_mask(
    ...
    // [B, seq_len] → [B, 1, 1, seq_len] → broadcasts with [seq_len, seq_len] local mask
    let expanded = mask
...
mod tests {
    ...
    // T-001: forward(input_ids, mask) → shape (1, seq_len, 768)
    #[test]
    #[serial]
    fn test_001_forward_produces_correct_shape() {
    ...
    // T-001: forward with different seq length
    #[test]
    #[serial]
    fn test_002_forward_different_seq_lengths() {
        let config = test_config();
        let mut model = ModernBert::new(&config).expect("create model");
    ...
    // T-002: load from SafeTensors + verify structure
    #[test]
    #[serial]
    #[ignore] // requires model download
    fn test_003_load_safetensors_correct_layers() {
    ...
    // T-005: non-existent model path → error
    #[test]
    #[serial]
    fn test_004_load_nonexistent_path_errors() {
        let config = test_config();
    ...
    // prepare_4d_attention_mask: unmasked → 0.0, masked → f32::MIN (~-3.4e38)
    // Uses f32::MIN (not NEG_INFINITY) because 0.0 * -inf = NaN (IEEE 754).
    #[test]
    #[serial]
    fn test_005_global_mask_values() {
    ...
    // all-ones mask → all zeros
    #[test]
    #[serial]
    fn test_006_global_mask_all_ones() {
    ...
    // RC-001b: local attention mask windowing
    #[test]
    #[serial]
    fn test_007_local_mask_window() {
        // seq_len=5, half_window=1 → positions within distance 1 are 0, others -inf
        ...
        // Check diagonal and neighbors are 0.0
        for i in 0..5usize {
            for j in 0..5usize {
                let val = data[i * 5 + j];
    ...
    // RC-001b: local mask with half_window=0 → diagonal only
    #[test]
    #[serial]
    fn test_008_local_mask_zero_window() {
